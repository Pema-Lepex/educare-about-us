  ...
  return (
    <div className="bg-white z-30">
      <section className="-mt-16 px-4 pb-10">
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-5 md:grid-cols-3">
          {features.map((f, idx) => {
            const specialSpan =
              idx === 3
                ? "md:col-span-1 md:col-start-1 md:justify-self-end md:max-w-[360px]"
                : idx === 4
                  ? "md:col-span-1 md:col-start-3 md:justify-self-start md:max-w-[360px]"
                  : "";
              ...
              <div
                ...
                onKeyDown={e => {
                  if (e.key === "Enter" || e.key === " ")
                    setActiveTab(themeToTab[f.theme]);
                }}
                className={`rounded-2xl border-2 bg-white p-5 shadow-[0_10px_20px_rgba(0,0,0,0.08)] ${borderByTheme[
                  f.theme
                ]} ${specialSpan} cursor-pointer`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3
                      className={`text-sm font-extrabold ${titleByTheme[
                        f.theme
                      ]}`}
                    >
                      {f.title}
      ...
      <section className="px-4 pb-16 pt-2">
        ...
        <div className="mx-auto mt-5 flex max-w-5xl flex-wrap justify-center gap-6 rounded-full border bg-blue-50 px-3 py-2">
          {([
            { key: "cheychey", label: "Chey Chey" },
            { key: "kids", label: "Educare Kids" },
            { key: "primary", label: "Primary Level" },
            ...
            <button
              ...
              className={[
                "rounded-full border px-12 py-2 text-xs font-semibold transition",
                activeTab === t.key
