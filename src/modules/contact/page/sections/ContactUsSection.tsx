import React from 'react';
import { MapPin, Phone, Mail, Clock } from 'lucide-react';
import { Contact } from 'assets';

export default function ContactSection() {
    return (
        <section className="py-16 px-4 max-w-7xl mx-auto space-y-12">
            {/* 1. Header Section */}
            <div className="text-center">
                <h2 className="text-3xl md:text-4xl font-black text-[#1b364d] tracking-tight uppercase mb-2">
                    CONTACT US
                </h2>
                <p className="text-[#1b364d]/80 text-lg">
                    We'd love to hear from you. Get in touch with our team!
                </p>
            </div>

            {/* 2. Top Content: Info and Office Image */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
                
                {/* Contact Info Card */}
                <div className="border border-customOriange-200 rounded-[2rem] p-8 md:p-12 shadow-sm bg-white">
                    <h3 className="text-orange-500 font-bold text-lg mb-8">Get In Touch</h3>
                    
                    <div className="space-y-8">
                        {/* Visit Us */}
                        <div className="flex gap-4">
                            <div className="p-3 bg-blue-50 rounded-full h-fit">
                                <MapPin className="w-5 h-5 text-[#1b364d]" />
                            </div>
                            <div>
                                <h4 className="font-bold text-gray-900">Visit Our Office</h4>
                                <p className="text-gray-600">Changzamtog, Thimphu</p>
                                <p className="text-gray-600">Bhutan</p>
                            </div>
                        </div>

                        {/* Call Us */}
                        <div className="flex gap-4">
                            <div className="p-3 bg-blue-50 rounded-full h-fit">
                                <Phone className="w-5 h-5 text-[#1b364d]" />
                            </div>
                            <div>
                                <h4 className="font-bold text-gray-900">Call Us</h4>
                                <p className="text-gray-600">[Phone Number]</p>
                                <p className="text-sm text-gray-400">Monday - Friday, 9:00 AM - 5:00 PM</p>
                            </div>
                        </div>

                        {/* Email Us */}
                        <div className="flex gap-4">
                            <div className="p-3 bg-blue-50 rounded-full h-fit">
                                <Mail className="w-5 h-5 text-[#1b364d]" />
                            </div>
                            <div>
                                <h4 className="font-bold text-gray-900">Email Us</h4>
                                <p className="text-gray-600">[Email Address]</p>
                                <p className="text-sm text-gray-400">We'll respond within 24 hours</p>
                            </div>
                        </div>

                        {/* Office Hours */}
                        <div className="flex gap-4">
                            <div className="p-3 bg-blue-50 rounded-full h-fit">
                                <Clock className="w-5 h-5 text-[#1b364d]" />
                            </div>
                            <div>
                                <h4 className="font-bold text-gray-900">Office Hours</h4>
                                <p className="text-gray-600">Monday - Friday: 9:00 AM - 5:00 PM</p>
                                <p className="text-gray-600">Saturday: 9:00 AM - 1:00 PM</p>
                                <p className="text-gray-600">Sunday: Closed</p>
                            </div>
                        </div>
                    </div>
                </div>

                {/* Office Building Image */}
                <div className="rounded-[2rem] overflow-hidden shadow-sm h-full">
                    <img 
                        src={Contact} // Replace with actual asset
                        alt="iBEST Institute Building" 
                        className="w-full h-full object-cover min-h-[400px]"
                    />
                </div>
            </div>

            {/* 3. Google Maps Section */}
            <div className="w-full h-[450px] rounded-[2rem] overflow-hidden border border-gray-200 shadow-sm">
                <iframe 
                    title="Educare Skill Location"
                    src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d14160.000000000002!2d89.63!3d27.47!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x39e194!2sThimphu%2C%20Bhutan!5e0!3m2!1sen!2sbt!4v1234567890123" 
                    width="100%" 
                    height="100%" 
                    style={{ border: 0 }} 
                    loading="lazy" 
                    referrerPolicy="no-referrer-when-downgrade"
                ></iframe>
            </div>
            <div className="w-full h-28"></div>
        </section>
    );
}